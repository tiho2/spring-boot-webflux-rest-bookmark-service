...
public class ResourceRest {
    ...
    public Mono<ResponseEntity<?>> user() {
        return Mono.just(ResponseEntity.ok(new Bookmark("Content for user",Boolean.FALSE)));
    }

    @RequestMapping(value = "/api/v1/resource/admin", method = RequestMethod.GET)
    @PreAuthorize("hasRole('ADMIN')")
    public Mono<ResponseEntity<?>> admin() {
        return Mono.just(ResponseEntity.ok(new Bookmark("Content for admin",Boolean.FALSE)));
    }
    ...
    public Mono<ResponseEntity<?>> userOrAdmin() {
        return Mono.just(ResponseEntity.ok(new Bookmark("Content for user or admin", Boolean.FALSE)));
    }
}